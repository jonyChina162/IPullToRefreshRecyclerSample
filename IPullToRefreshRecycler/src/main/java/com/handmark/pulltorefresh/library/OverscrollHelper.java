...
public final class OverscrollHelper {
    ...
    public static void overScrollBy(final PullToRefreshBase<?> view, final int deltaX, final int scrollX,
                                    final int deltaY, final int scrollY, final int scrollRange, final int fuzzyThreshold,
                                    final float scaleFactor, final boolean isTouchEvent, final boolean isWebView) {
        if (!isWebView) {
            overScrollBy(view, deltaX, scrollX, deltaY, scrollY, scrollRange, fuzzyThreshold, scaleFactor, isTouchEvent);
            return;
        }
        ...
        if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
            ...
            // Check that Pull-to-Refresh is enabled, and the event isn't from
            // touch
//			if (mode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0) {//修改前版本
            if (mode.permitsPullToRefresh() && isTouchEvent && deltaValue != 0) {//为下拉做修改，以及else分支
                final int newScrollValue = (deltaValue + scrollValue);
