...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private Mode mTemporaryMode = Mode.BOTH;
    ...
    public void setTemporaryMode(Mode mode, String text) {
        if (mode != mTemporaryMode) {
            if (DEBUG) {
                Log.d(LOG_TAG, "Setting temporary mode to: " + mode);
            }
            mTemporaryMode = mode;
            mHeaderLayout.setTemporaryMode(mode, text);
            mFooterLayout.setTemporaryMode(mode, text);
        }
    }
    ...
    private void callRefreshListener() {
        if (null != mOnRefreshListener) {
            ...
            if (mCurrentMode == Mode.PULL_FROM_START) {
                if (mTemporaryMode == Mode.PULL_FROM_END) {
                    onRefreshComplete();
                } else {
                    mOnRefreshListener2.onPullDownToRefresh(this);
                }
            } else if (mCurrentMode == Mode.PULL_FROM_END) {
                if (mTemporaryMode == Mode.PULL_FROM_START) {
                    onRefreshComplete();
                } else {
    ...
    private void pullEvent() {
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(newScrollValue)) {
                ...
                switch (mCurrentMode) {
                    case PULL_FROM_END:
                        if (mTemporaryMode == Mode.BOTH || mTemporaryMode == Mode.PULL_FROM_END) {
                            setState(State.RELEASE_TO_REFRESH);
                        }
                        break;
                    case PULL_FROM_START:
                    default:
                        if (mTemporaryMode == Mode.BOTH || mTemporaryMode == Mode.PULL_FROM_START) {
                            setState(State.RELEASE_TO_REFRESH);
                        }
                        break;
                }
    ...
    //for webView
    public void setHeaderRefreshing(boolean b) {
        if (getRefreshableView() instanceof WebView) {
            if (b) {
