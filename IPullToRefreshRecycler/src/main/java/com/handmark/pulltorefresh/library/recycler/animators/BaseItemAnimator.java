...
import com.handmark.pulltorefresh.library.recycler.animators.holder.AnimateViewHolder;
import com.handmark.pulltorefresh.library.recycler.animators.internal.DoAnimationViewPropertyListener;
...
/**
 * add two methods[doAddAnimation,doRemoveAnimation] which allow user to start
 * animation directly and not notify recyclerView; add two
 * listeners[mDoAddAnimationViewPropertyListener,
 * mDoRemoveAnimationViewPropertyListener]which provides more convenience to
 * control the viewHolder's animation.
 *
 */
public abstract class BaseItemAnimator extends RecyclerView.ItemAnimator {
	 ...
	 * none pending/running, call #dispatchAnimationsFinished() to notify any
	 * listeners.
	 */
	...
	public class DefaultAddVpaListener extends VpaListenerAdapter {
		...
		public DefaultAddVpaListener(ViewHolder mViewHolder) {
			this.mViewHolder = mViewHolder;
		}

		@Override
		public void onAnimationStart(View view) {
			if (mDoAddAnimationViewPropertyListener == null
					|| !mDoAddAnimationViewPropertyListener.onAnimationStart(view)) {
	...
	protected class DefaultRemoveVpaListener extends VpaListenerAdapter {
		...
		public DefaultRemoveVpaListener(ViewHolder mViewHolder) {
			this.mViewHolder = mViewHolder;
		}

		@Override
		public void onAnimationStart(View view) {
			if (mDoRemoveAnimationViewPropertyListener == null
					|| !mDoRemoveAnimationViewPropertyListener.onAnimationStart(view)) {
		...
		public void onAnimationCancel(View view) {
			if (mDoRemoveAnimationViewPropertyListener == null
					|| !mDoRemoveAnimationViewPropertyListener.onAnimationCancel(view)) {
				ViewHelper.clear(view);
			}
		}
		...
		public void onAnimationEnd(View view) {
			if (mDoRemoveAnimationViewPropertyListener == null
					|| !mDoRemoveAnimationViewPropertyListener.onAnimationEnd(view)) {
				ViewHelper.clear(view);
	...
	public void doAddAnimation(ViewHolder holder) {
		animateAddImpl(holder);
	}

	public void doRemoveAnimation(ViewHolder holder) {
		animateRemoveImpl(holder);
	}

	private DoAnimationViewPropertyListener mDoAddAnimationViewPropertyListener;
	private DoAnimationViewPropertyListener mDoRemoveAnimationViewPropertyListener;

	public void setDoAddAnimationViewPropertyListener(
			DoAnimationViewPropertyListener mDoAddAnimationViewPropertyListener) {
		this.mDoAddAnimationViewPropertyListener = mDoAddAnimationViewPropertyListener;
	}

	public void setDoRemoveAnimationViewPropertyListener(
			DoAnimationViewPropertyListener mDoRemoveAnimationViewPropertyListener) {
		this.mDoRemoveAnimationViewPropertyListener = mDoRemoveAnimationViewPropertyListener;
	}
}
